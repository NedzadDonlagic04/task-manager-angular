...
export class ViewTasksTable implements OnInit, AfterViewInit {
  ...
  showDeleteDialog(taskId?: string): void {
    let batchDelete: boolean = this.selectedTasks.selected.length > 0;
    ...
    let dialogRef = this.dialog.open(YesNoDialog, {
      ...
      data: {
        title: 'Delete',
        message: batchDelete
          ? `Are you sure you want to delete ${this.selectedTasks.selected.length} tasks`
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && batchDelete) this.batchDelete();
